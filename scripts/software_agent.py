...
def payload_for(spec,prompt):
 ...
 for x in spec.get('parameters',[]):
  ...
  elif l in {'system','system_prompt'}: p[n]='Rigorous software analysis. CLAIM<=EVIDENCE. Never invent executions.'
...
prompt=f'''You are role {role} in CEREBRON Farm 17 Computing Software. Focus: {focus}. Work as a software/computing specialist. Distinguish design, implementation, test, benchmark, deployment and production evidence. Do not invent executions. Identify assumptions, failure modes, security risks, observability needs and measurable validation steps. Return concise sections: FINDINGS, EVIDENCE, RISKS, TESTS, UNKNOWN, VERDICT.'''
ok,text,meta=invoke(model,prompt)
out={'role':role,'model':model,'focus':focus,'success':bool(ok),'inference_success':bool(ok),'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','result':text if ok else None,'error':None if ok else meta.get('error'),'meta':meta}
pathlib.Path('out').mkdir(exist_ok=True); pathlib.Path(f'out/{role}.json').write_text(json.dumps(out,ensure_ascii=False,indent=2))
